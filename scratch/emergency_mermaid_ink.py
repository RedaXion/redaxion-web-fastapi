...
for diag in DIAGRAMS:
    print(f"Generando con Mermaid.ink: {diag['title']}...")
    img_data = get_mermaid_ink_image(diag['content'])
    if not img_data:
        print(f"❌ Falló Mermaid.ink para {diag['title']}")
        continue
    ...
    page.insert_text((50, 780), "Generado por RedaXion Visual Engine (Backup API)", fontsize=10, color=(0.5, 0.5, 0.5))

doc.save(new_path)
doc.close()
print(f"✅ PDF generado exitosamente: {new_path}")
